...
void HTTPRequest::addData(const char* buf, int len)
{
...
int HTTPRequest::setURL(const string &url)
{
	m_url = url;
	return 0;
}
...
int HTTPRequest::setUserAgent(const string &userAgent)
{
...
int HTTPRequest::setHTTPHeader(const string &headerName, const string &header_content)
{
	m_httpHeaders.push_back(make_pair(headerName, headerContent));
	return 0;
}

string HTTPRequest::getHTTPHeader(const string &headerName)
{
...
{
	for(auto it = httpHeaderVector->begin(); it != httpHeaderVector->end(); it++){
